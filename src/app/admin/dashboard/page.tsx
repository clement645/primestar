...
import { TrendChart, WorkerPerformanceChart } from "@/components/admin/AdminCharts";
...
export default async function AdminOverviewPage({
  ...
  const conversionRate = totalClicks > 0 ? (totalWhatsapp / totalClicks) * 100 : 0;
  ...
  const workerPerf = workers
    .map((w) => ({
      ...
      rate: w._count.referralClicks > 0 ? (w._count.whatsappConversions / w._count.referralClicks) * 100 : 0,
    }))
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h1 className="font-heading text-3xl font-extrabold text-brand-dark">Overview</h1>
            ...
            .map((key) => (
              <Link
                ...
                className={`rounded-full px-4 py-1.5 text-sm font-medium ${
                  range === key ? "bg-brand-dark text-white" : "bg-brand-lighter text-brand-dark"
                }`}
              >
                {RANGE_LABELS[key]}
              </Link>
      ...
      <div className="mt-8 rounded-2xl border border-brand-lighter bg-white p-6">
        <h2 className="font-heading text-lg font-bold text-brand-dark">Worker Performance</h2>
        <div className="mt-4">
          <WorkerPerformanceChart data={workerPerf.slice(0, 8).map((w) => ({ name: w.code, clicks: w.clicks, whatsapp: w.whatsapp }))} />
        </div>
            ...
            <tbody>
              {workerPerf.map((w) => (
                <tr key={w.id} className="border-b border-brand-lighter/60">
                  <td className="py-2.5 font-medium text-brand-dark">{w.name}</td>
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-2xl border border-brand-lighter bg-white p-5">
      ...
      <p className="mt-1 font-heading text-2xl font-extrabold text-brand-dark">{value}</p>
    </div>
  );
}
